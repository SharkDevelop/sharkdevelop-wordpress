...
class Ezfc_paypal {
	public static function checkout($ezfc, $pp_ref_id) {
		$paymentAmount    = round($_SESSION["Payment_Amount"], 2);
		$paymentType      = "Sale";
		...
		$custom_data      = "&PAYMENTREQUEST_0_CUSTOM={$pp_ref_id}";

		$returnURL = apply_filters("ezfc_paypal_return_url", $returnURL, $ezfc->submission_data);
		$cancelURL = apply_filters("ezfc_paypal_cancel_url", $cancelURL, $ezfc->submission_data);

		$resArray = CallShortcutExpressCheckout($paymentAmount, $currencyCodeType, $paymentType, $returnURL, $cancelURL, $custom_data);
	...
	public static function confirm() {
		$finalPaymentAmount = round($_SESSION["Payment_Amount"], 2);
